...
#### 1. 侧边栏评论链接修复
- **sidebar.php**: 修复未登录时独立页面评论链接不显示的问题，简化判断逻辑并添加 permalink 兜底

#### 2. 返回顶部功能修复
- **main.min.js**: 修复动画参数错误、接近顶部卡死、pjax切换后失效、移动端无法打断动画等问题

## v3.2.8 (2026-04-08)

#### 1. 主题设置备份功能优化
- **functions.php**: 添加操作确认对话框防止误操作
- **functions.php**: 使用不同颜色区分按钮类型，提高操作辨识度
- **functions.php**: 实现版本管理，自动保留最近2个备份
- **functions.php**: 添加备份列表显示，方便查看现有备份
- **功能特点**: 更安全、更直观的备份管理界面，大幅减少误操作风险

## v3.2.7 (2026-03-20)

#### 1. 导航下拉菜单Bug修复
- **style.min.css**: 为 `.menu-parent ul` 添加 `z-index:999`，修复下拉菜单被侧边栏遮挡的问题
- **问题描述**: 当侧边栏开启时，鼠标从菜单项缓慢滑向下拉菜单，经过间隙时菜单异常消失

## v3.2.6 (2026-03-17)

#### 1. 文章时效性过期提醒功能
- **functions.php**: 新增文章过期提醒天数设置选项，默认180天
- **functions.php**: 新增 `expireNotice` 函数，计算文章发表天数与设置天数的对比
- **post.php**: 添加文章时效性过期提醒显示，显示"本文已超过xx天未更新，部分内容可能具有时效性，请注意核实最新情况。"
- **功能特点**: 主题设置中可自定义提醒天数，到期后在文章顶部显示提示信息

## v3.2.5 (2026-03-15)

#### 1. 回复可见功能优化
- **post.php**: 按需执行“回复可见”模块[hidden][/hidden]标签的数据库查询，避免每次加载都执行查询，提升性能
- **功能特点**: 先用 strpos 判断文章内容中是否包含 [hidden]，只有包含时才去查库验证权限。strpos 在 PHP 中执行速度极快。


## v3.2.4 (2026-03-06)

#### 1. 图片懒加载功能优化
- **functions.php**: 优化 `lazyLoadImages` 函数，首图片不懒加载，优化 CLS 和 SEO 表现
- **functions.php**: 优化 `postThumb` 函数，统一懒加载图片格式，添加 `loading="lazy"` 属性
- **footer.php**: 优化 lazysizes 库加载逻辑，按需加载 JS 文件，避免无懒加载图片的页面加载不必要的资源
- **功能特点**: 更智能的懒加载策略，首屏图片立即加载优化 CLS 和 SEO，后续图片延迟加载，且只在需要时加载 lazysizes 库，提升性能和用户体验

## v3.2.3 (2026-03-04)

#### 1. 图片懒加载功能
- **functions.php**: 添加图片懒加载开关选项，默认关闭，用户可根据需要启用
- **functions.php**: 修改themeInit和postThumb函数，根据LazyLoad设置决定是否应用懒加载
- **footer.php**: 根据LazyLoad设置决定是否加载lazysizes库，避免不必要的资源加载
- **功能特点**: 支持自动检测视口内的图片并延迟加载，提升页面加载速度和用户体验

#### 2. 回复可见功能优化
- **post.php**: 修复代码块中的[hidden]标签被误处理的问题，保护代码块内容不被替换
- **functions.php**: 修改myyodux类，在处理摘要时也保护代码块中的[hidden]标签
- **功能特点**: 代码块（```和<pre>）中的[hidden]标签现在会被正确保留，不会触发回复可见功能

## v3.2.2 (2026-02-28)

#### 1. 侧边栏自定义代码功能
- **functions.php**: 添加侧边栏自定义代码设置选项，包括开关、内容输入和位置选择
- **sidebar.php**: 支持在侧边栏不同位置显示自定义代码
- **功能特点**: 支持HTML、CSS和JavaScript代码，可自由选择显示位置，提升主题扩展性
