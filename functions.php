...
error_reporting(0);
define('INITIAL_VERSION_NUMBER', '3.3.0');
...
function themeConfig($form) {
	...
	$defaultCover = new Typecho_Widget_Helper_Form_Element_Text('defaultCover', NULL, NULL, _t('SEO-文章封面图片'), _t('【网站不显示】用于没有自定义封面的页面，通过 og:image 标签提供给搜索引擎和社交平台，建议尺寸1200x630px。不填则使用内置默认封面。'));
	$form->addInput($defaultCover);

	$homeCover = new Typecho_Widget_Helper_Form_Element_Text('homeCover', NULL, NULL, _t('SEO-首页封面图片'), _t('【网站不显示】专门用于首页，通过 og:image 标签提供给搜索引擎和社交平台，建议使用品牌形象图片。不填则使用内置默认封面图片。'));
	$form->addInput($homeCover);

	$autoFetchCover = new Typecho_Widget_Helper_Form_Element_Radio('autoFetchCover', 
	array(1 => _t('启用'),
	0 => _t('关闭')),
	1, _t('SEO-自动抓取文章封面图'), _t('【网站不显示】自动抓取文章正文中的第一张图片作为封面图，通过 og:image 标签提供给搜索引擎和社交平台。'));
	...
	$ShowLinks = new Typecho_Widget_Helper_Form_Element_Checkbox('ShowLinks', array('footer' => _t('页脚'), 'sidebar' => _t('侧边栏')), array('footer'), _t('首页显示链接'));
	$form->addInput($ShowLinks->multiMode());

	$ShowWhisper = new Typecho_Widget_Helper_Form_Element_Checkbox('ShowWhisper', array('index' => _t('首页'), 'sidebar' => _t('侧边栏')), array('sidebar'), _t('显示最新的"轻语"'));
